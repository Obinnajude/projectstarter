...
#Step: transforms.Resize(256)
#Purpose: This step resizes the shorter side of the image to 256 pixels while maintaining the aspect ratio. This ensures that all images have a consistent size for further processing.
#Crop:
#Training Set: transforms.RandomCrop(224)
#Purpose: Randomly crops a 224x224 patch from the resized image. This introduces variability in the training data, helping the model generalize better.
#Validation and Test Sets: transforms.CenterCrop(224)
#Purpose: Crops a 224x224 patch from the center of the resized image. This ensures consistency in the validation and test sets, providing a stable evaluation metric.
#Data Augmentation (Training Set Only):
#Step: transforms.RandomHorizontalFlip()
#Purpose: Randomly flips the image horizontally with a probability of 0.5. This augmentation helps the model become invariant to the horizontal orientation of objects in the images.
#Convert to Tensor:

#Step: transforms.ToTensor()
#Purpose: Converts the image from a PIL Image or numpy.ndarray to a PyTorch tensor. This step is necessary for the image to be processed by PyTorch models.

#Normalize:
#Step: transforms.Normalize(mean, std)
#Purpose: Normalizes the image tensor using the computed mean and standard deviation of the dataset. This step ensures that the pixel values have a mean of 0 and a standard deviation of 1, which helps in faster and more stable training.
